...
export default function RibbonShader3() {
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      // 이미지 배경 그라데이션 색상 (네온 라임 → 옐로 → 오렌지 → 핑크)
      vec3 getColor(float t, vec2 uv) {
        ...
        // 이미지의 배경 그라데이션 색상 (왼쪽 → 오른쪽)
        vec3 c1 = vec3(0.85, 0.45, 0.75); // 핑크 (왼쪽 끝)
        vec3 c2 = vec3(0.95, 0.65, 0.50); // 연한 오렌지/피치
        vec3 c3 = vec3(1.00, 0.90, 0.40); // 밝은 옐로우
        vec3 c4 = vec3(0.90, 1.00, 0.35); // 네온 라임 그린
        vec3 c5 = vec3(0.70, 0.95, 0.40); // 라임 그린
        vec3 c6 = vec3(0.55, 0.85, 0.50); // 그린
        vec3 c7 = vec3(0.45, 0.75, 0.60); // 청록색
