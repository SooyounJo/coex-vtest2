...
export default function ShaderBubble9({ styleType = 6, cameraMode = 'default' }) {
  ...
  // 큰 도넛 형태
  const torusRadius = Math.min(viewport.width, viewport.height) * 0.8 // 매우 큰 도넛
  const tubeRadius = torusRadius * 0.35 // 두꺼운 튜브

  return (
    <>
      <mesh ref={meshRef} position={[0, 0, 0]} rotation={[Math.PI / 4, 0, 0]}>
        <torusGeometry args={[torusRadius, tubeRadius, 64, 128]} />
