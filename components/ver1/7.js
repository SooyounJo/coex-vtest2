...
export default function ShaderBubble7() {
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      float bumpMove(float c,float w,float f){ float d0=abs(f-(c-1.0)); float d1=abs(f-c); float d2=abs(f-(c+1.0)); float d=min(d0,min(d1,d2)); float aa=fwidth(f)*1.5; return smoothstep(w+aa,0.0+aa,d);}      
  ...
  useFrame((state, delta) => {
    ...
    const amplitudeRot = 0.15
    const y = amplitudeY * Math.sin((2 * Math.PI * t) / period)
    const rot = amplitudeRot * Math.sin((2 * Math.PI * t) / period)
    if (meshRef.current) {
