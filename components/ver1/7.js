...
export default function SharpBubble({ styleType = 7 }) {
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      float sharpBumpMove(float center,float width,float f){ float d0=abs(f-(center-1.0)); float d1=abs(f-center); float d2=abs(f-(center+1.0)); float d=min(d0,min(d1,d2)); float aa=fwidth(f)*0.1; return smoothstep(width+aa,0.0+aa,d);}      
      vec3 bandWeights(float f){ float width=0.05; float y=sharpBumpMove(0.18,width,f); float p=sharpBumpMove(0.52,width,f); float u=sharpBumpMove(0.86,width,f); return vec3(y,p,u);}      
      void main(){
        ...
        float blurAmount=0.001; float f1=topness*1.8+phase+totalRipple+totalElastic; float f2=topness*1.8+phase+blurAmount+totalRipple*0.8+totalElastic*0.6; float f3=topness*1.8+phase+(blurAmount*1.5)+totalRipple*0.6+totalElastic*0.4;
        float perturb=0.001*n2(vUv*1.5+time*0.05); vec3 w1=bandWeights(f1+perturb); vec3 w2=bandWeights(f2+perturb*0.8); vec3 w3=bandWeights(f3+perturb*0.6);
        float wobble1=0.9998+0.0001*n2(vUv*2.2+time*0.06); float wobble2=0.9998+0.0001*n2(vUv*2.2+time*0.06+1.7); float wobble3=0.9998+0.0001*n2(vUv*2.2+time*0.06+3.1); w1*=wobble1; w2*=wobble2; w3*=wobble3;
