  ...
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        vec3 peach=vec3(1.00,0.95,0.80); vec3 pink=vec3(1.00,0.80,0.95); vec3 purple=vec3(0.90,0.75,1.00);
        ...
        float blurAmount=0.08; float f1=topness*1.8+phase+totalRipple+totalElastic; float f2=topness*1.8+phase+blurAmount+totalRipple*0.8+totalElastic*0.6; float f3=topness*1.8+phase+(blurAmount*1.5)+totalRipple*0.6+totalElastic*0.4;
        ...
        vec3 cY=vec3(0.60,0.90,1.00); vec3 cP=vec3(0.40,0.70,1.00); vec3 cU=vec3(0.20,0.50,0.90);
        ...
        vec3 lit=base; lit=mix(lit,flowColor,flowMaskAvg*0.8);
        vec3 rippleColor=vec3(0.3,0.7,1.0)*totalRipple*0.4; vec3 elasticColor=vec3(0.2,0.6,0.9)*totalElastic*0.3; lit+=rippleColor+elasticColor;
        vec3 V=vec3(0.0,0.0,1.0); float fres=pow(1.0 - max(dot(N,V),0.0),2.6); vec3 rimGlow=vec3(0.2,0.6,1.0)*fres*0.5; float softHalo=smoothstep(0.34,0.10,r)*0.13; vec3 glow=rimGlow + vec3(0.3,0.7,1.0)*softHalo; lit+=glow;
        lit+=vec3(0.1,0.5,0.8)*(1.0-topness)*0.2; vec3 gray=vec3(dot(lit,vec3(0.299,0.587,0.114))); lit=mix(gray,lit,2.2); lit=pow(lit,vec3(0.85)); lit*=1.15; lit=mix(lit,vec3(1.0),0.05); lit=clamp(lit,0.0,1.1);
