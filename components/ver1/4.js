  ...
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        float blurAmount=0.03; float f1=topness*1.8+phase+totalRipple+totalElastic; float f2=topness*1.8+phase+blurAmount+totalRipple*0.8+totalElastic*0.6; float f3=topness*1.8+phase+(blurAmount*1.5)+totalRipple*0.6+totalElastic*0.4;
        ...
        vec3 lit=base; lit=mix(lit,flowColor,flowMaskAvg*0.45);
        vec3 rippleColor=vec3(0.8,0.4,0.6)*totalRipple*0.4; vec3 elasticColor=vec3(0.8,0.3,0.7)*totalElastic*0.3; lit+=rippleColor+elasticColor;
        vec3 V=vec3(0.0,0.0,1.0); float fres=pow(1.0 - max(dot(N,V),0.0),2.6); vec3 rimGlow=vec3(0.8,0.3,0.7)*fres*0.5; float softHalo=smoothstep(0.34,0.10,r)*0.13; vec3 glow=rimGlow + vec3(0.8,0.4,0.8)*softHalo; lit+=glow;
        lit+=vec3(0.8,0.2,0.6)*(1.0-topness)*0.2; vec3 gray=vec3(dot(lit,vec3(0.299,0.587,0.114))); lit=mix(gray,lit,2.0); lit=pow(lit,vec3(0.9)); lit*=1.10; lit=mix(lit,vec3(1.0),0.04); lit=clamp(lit,0.0,1.05);
