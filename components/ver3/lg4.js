...
export default function AgenticBubble({ styleType = 6, cameraMode = 'default' }) {
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        // 새로운 컬러 팔레트: 빨강-마젠타-보라-라벤더-흰색
        vec3 red=vec3(1.0,0.2,0.2); vec3 darkRed=vec3(0.7,0.1,0.1); vec3 magenta=vec3(1.0,0.0,0.8); vec3 purple=vec3(0.6,0.0,1.0); vec3 lavender=vec3(0.9,0.7,1.0); vec3 white=vec3(1.0,1.0,1.0);
        vec3 base=mix(red,darkRed,clamp(0.5+0.5*topness,0.0,1.0)); 
        base=mix(base,magenta,smoothstep(0.0,0.2,1.0-topness));
        base=mix(base,purple,smoothstep(0.2,0.4,1.0-topness));
        base=mix(base,lavender,smoothstep(0.4,0.6,1.0-topness));
        base=mix(base,white,smoothstep(0.6,0.8,1.0-topness));
        ...
        // 새로운 플로우 컬러: 빨강-마젠타-보라 계열
        vec3 cY=vec3(1.0,0.1,0.1); vec3 cP=vec3(1.0,0.0,0.6); vec3 cU=vec3(0.8,0.0,1.0);
        ...
        vec3 rippleColor=vec3(1.0,0.2,0.3)*totalRipple*0.2; vec3 elasticColor=vec3(0.9,0.1,0.5)*totalElastic*0.15; lit+=rippleColor+elasticColor;
        vec3 V=vec3(0.0,0.0,1.0); float fres=pow(1.0 - max(dot(N,V),0.0),2.6); vec3 rimGlow=vec3(1.0,0.2,0.4)*fres*0.3; float softHalo=smoothstep(0.34,0.10,r)*0.08; vec3 glow=rimGlow + vec3(1.0,0.3,0.6)*softHalo; lit+=glow;
        lit+=vec3(1.0,0.1,0.3)*(1.0-topness)*0.1; vec3 gray=vec3(dot(lit,vec3(0.299,0.587,0.114)));
  ...
  const radius = Math.min(v.width, v.height) * (isVer3 ? 1.8 : 0.9)
