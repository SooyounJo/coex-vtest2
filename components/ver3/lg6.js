...
export default function AgenticBubble({ styleType = 6, cameraMode = 'default' }) {
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        float f1=topness*1.8+phase+totalRipple+totalElastic; float f2=topness*1.8+phase+totalRipple*0.8+totalElastic*0.6; float f3=topness*1.8+phase+totalRipple*0.6+totalElastic*0.4;
        ...
        vec3 V=vec3(0.0,0.0,1.0); float fres=pow(1.0 - max(dot(N,V),0.0),2.6); vec3 rimGlow=vec3(0.8,0.3,0.7)*fres*0.3; lit+=rimGlow;
  ...
  const radius = Math.min(v.width, v.height) * (isVer3 ? 0.8 : 0.4)
  ...
  const boxGeometry = useMemo(() => {
    const geometry = new THREE.BoxGeometry(radius * 1.2, radius * 0.8, radius * 0.6)
    return geometry
  }, [radius])

  return (
    <>
      <mesh ref={meshRef} position={[0, 0, 0]} rotation={[Math.PI / 3, 0, 0]} geometry={boxGeometry} material={material} />
    </>
  )
}
