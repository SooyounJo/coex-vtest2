  ...
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        // 3D 위치 계산 (UV를 3D로 변환)
        vec3 pos3D = vec3((vUv - 0.5) * 2.0, 0.0);
        pos3D.z = sin(vUv.x * 3.14159) * sin(vUv.y * 3.14159) * 0.5;
        
        // 메타볼 애니메이션 - 여러 구체들이 움직이며 합쳐짐
        float metaballField = 0.0;
        
        // 메인 구체 (중앙)
        vec3 center1 = vec3(0.0, 0.0, 0.0);
        float radius1 = 0.8 + 0.2 * sin(time * 0.5);
        metaballField += metaball(pos3D, center1, radius1);
        
        // 회전하는 구체들
        float angle1 = time * 0.8;
        vec3 center2 = vec3(cos(angle1) * 0.6, sin(angle1) * 0.6, sin(time * 0.3) * 0.3);
        float radius2 = 0.4 + 0.1 * sin(time * 0.7);
        metaballField += metaball(pos3D, center2, radius2);
        
        float angle2 = time * 1.2 + 2.0;
        vec3 center3 = vec3(cos(angle2) * 0.5, sin(angle2) * 0.5, cos(time * 0.4) * 0.4);
        float radius3 = 0.3 + 0.1 * cos(time * 0.9);
        metaballField += metaball(pos3D, center3, radius3);
        
        float angle3 = time * 0.6 + 4.0;
        vec3 center4 = vec3(cos(angle3) * 0.7, sin(angle3) * 0.7, sin(time * 0.6) * 0.2);
        float radius4 = 0.35 + 0.1 * sin(time * 1.1);
        metaballField += metaball(pos3D, center4, radius4);
        
        // 메타볼 필드 기반 마스크
        float threshold = 1.2;
        float metaballMask = smoothstep(threshold - 0.1, threshold + 0.1, metaballField);
