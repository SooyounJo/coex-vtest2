...
export default function Type5() {
  ...
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        float blurAmount = isVer3 > 0.5 ? 0.06 : 0.04;
        float scale = 1.8;
        ...
        lit = clamp(lit,0.0,1.0);
         ...
         float alpha = 0.45*edgeFeather + fres*0.06; // 더 투명하게 조정
         alpha = clamp(alpha,0.0,0.55); // 최대 투명도도 더 낮춤
  ...
  // 우측 아래에 위치 (Type5) - 뒤쪽으로 이동
  const xPosition = isVer3 ? v.width * 0.3 : v.width * 0.25
  const yPosition = isVer3 ? -v.height * 0.4 : -v.height * 0.35
  const zPosition = -2.0 // 뒤쪽으로 이동

  useFrame((state, delta) => {
    material.uniforms.time.value += delta
    miniMaterial.uniforms.time.value = state.clock.elapsedTime
      ...
      if (ref.current) {
        ...
        if (cycle < 0.4) {
          // 0-2.4초: 로딩하듯 돌아가는 애니메이션
           ...
           const z = Math.sin(time * 3 + index) * 0.2 + zPosition + 0.8 // 메인 구보다 확실히 앞쪽
          ...
          const easeIn = t * t * (3.0 - 2.0 * t)
           ...
           const z = Math.sin(time * 3 + index) * 0.2 * (1.0 - easeIn) + zPosition + 0.8 // 메인 구보다 확실히 앞쪽
          ...
          const easeOut = t * t * (3.0 - 2.0 * t)
           ...
           const z = Math.sin(time * 3 + index) * 0.2 * easeOut + zPosition + 0.8 // 메인 구보다 확실히 앞쪽
  ...
  // 미니 구들의 위치 계산 (메인 구 중심 기준)
  const miniRadius = radius * 0.06

  return (
    <>
      {/* 메인 구 (우측 아래, 뒤쪽) */}
      <mesh ref={meshRef} position={[xPosition, yPosition, zPosition]}>
        <sphereGeometry args={[radius, 256, 256]} />
        <primitive object={material} attach="material" />
      </mesh>
      
      {/* 미니 구 5개 (메인 구 중심에서 애니메이션, 살짝 앞쪽) */}
      {miniRefs.map((ref, index) => (
        <mesh key={index} ref={ref} position={[xPosition, yPosition, zPosition]} visible={false}>
