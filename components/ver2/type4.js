...
export default function Type4() {
  const material = useMemo(() => new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        ...
        float blurAmount = isVer3 > 0.5 ? 0.06 : 0.04; // ver3 모달에서 더 강한 블러
        float scale = 1.8;
  ...
  const baseRadius = Math.min(v.width, v.height) * (isVer3 ? 0.8 : 0.33)
  const margin = isVer3 ? v.height * 0.01 : v.height * 0.035
  const yBottom = isVer3 ? -v.height / 2 + baseRadius * 0.6 + margin : -v.height / 2 + baseRadius + margin
  ...
  useFrame((state, delta) => {
    ...
    // 6초 주기 애니메이션 (변형 1초 + 퍼즈 1초) * 3단계
    const cycle = (time % 6.0) / 6.0
    
    let targetScale = 1.0
    
    if (cycle < 0.167) {
      // 0-1초: 큰 상태에서 작게 (빠른 변형)
      const t = cycle / 0.167
      const springT = 1.0 - Math.pow(1.0 - t, 3) // 빠른 스프링 이징
      targetScale = 1.0 - springT * 0.7 // 1.0 → 0.3
    } else if (cycle < 0.333) {
      // 1-2초: 작은 상태 퍼즈
      targetScale = 0.3
    } else if (cycle < 0.5) {
      // 2-3초: 작은 상태에서 크게 (빠른 변형)
      const t = (cycle - 0.333) / 0.167
      const springT = 1.0 - Math.pow(1.0 - t, 3) // 빠른 스프링 이징
      targetScale = 0.3 + springT * 0.7 // 0.3 → 1.0
    } else if (cycle < 0.667) {
      // 3-4초: 큰 상태 퍼즈
      targetScale = 1.0
    } else if (cycle < 0.833) {
      // 4-5초: 큰 상태에서 작게 (빠른 변형)
      const t = (cycle - 0.667) / 0.167
      const springT = 1.0 - Math.pow(1.0 - t, 3) // 빠른 스프링 이징
      targetScale = 1.0 - springT * 0.7 // 1.0 → 0.3
    } else {
      // 5-6초: 작은 상태 퍼즈
    ...
    // 스프링 기반 스케일 애니메이션
    const stiffness = 25.0 // 높은 강성으로 빠른 반응
    const damping = 0.85 // 적당한 감쇠로 찰진 느낌
    
    const currentScale = material.uniforms.scaleFactor.value
    const acceleration = (targetScale - currentScale) * stiffness
    scaleVelocityRef.current += acceleration * dt
    scaleVelocityRef.current *= damping
    const newScale = currentScale + scaleVelocityRef.current * dt
    
    // 위치는 중앙에 고정 (움직임 없음)
    material.uniforms.scaleFactor.value = newScale
    material.uniforms.positionX.value = 0.0
    material.uniforms.positionY.value = 0.0
  ...
  return (
    <>
      <mesh ref={meshRef} position={[0, yBottom, 0]}>
        <sphereGeometry args={[baseRadius, 256, 256]} />
